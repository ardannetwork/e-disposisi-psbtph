...
import { ShieldCheck, UserCheck, Sparkles, AlertCircle, Key, LogOut, Wallet, Sun, Moon, Menu, User } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ onOpenFirebaseModal, onToggleSidebar }) => {
  const { currentUser, logout, usersList, isFirebaseActive, theme, toggleTheme, updateCurrentUser } = useAuth();
  ...
  return (
    <>
      <header className="sticky top-0 z-[60] bg-slate-900/80 backdrop-blur-md border-b border-slate-800 text-slate-100 px-3 sm:px-4 lg:px-8 py-2 sm:py-3 transition-all overflow-x-auto">
    <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 sm:gap-3 md:gap-4 flex-nowrap">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3">
          ...
          <button
            ...
            className={`hidden md:flex items-center gap-1.5 text-xs px-2 sm:px-3 py-1.5 rounded-lg border transition-all ${
              isFirebaseActive
          ...
          >
            ...
            <span className="truncate max-w-[120px]">{isFirebaseActive ? 'Firebase Terhubung' : 'Demo Local (Set Firebase)'}</span>
          </button>

            {/* ROLE BADGE */}
            <div className="flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-2 md:px-3 py-1.5 rounded-xl bg-slate-800/90 border border-slate-700 text-xs shadow-md">
              <div className="flex items-center gap-1 sm:gap-1.5">
                {currentUser?.role === 'admin' && (
                  <ShieldCheck className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-400 flex-shrink-0" />
                )}
                {currentUser?.role === 'operator' && (
                  <UserCheck className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-cyan-400 flex-shrink-0" />
                )}
                ...
                <div className="text-left hidden sm:block min-w-0">
                  <div className="font-semibold capitalize text-slate-200 truncate">
          ...
          <button
            onClick={toggleTheme}
            className="flex items-center gap-1.5 bg-slate-800/90 hover:bg-slate-700/90 border border-slate-700 text-slate-300 px-2 sm:px-2.5 py-1.5 rounded-xl text-xs font-semibold transition-colors"
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 text-rose-300 px-2 sm:px-3 py-1.5 rounded-xl text-xs font-semibold transition-colors"
